...
export function validateForm(oldFormState) {
  ...
  const guestValidationResult = validateGuests(formState.guests.value, formState.guests.maxTableSize);
...
export function validateDate(dateString) {
  ...
  if (!isValid(date))
    result.setInvalid("You must select a valid date!");
...
export function validateTime(time) {
  ...
  if (!re.test(time))
    result.setInvalid("Time is not in the correct format.");
